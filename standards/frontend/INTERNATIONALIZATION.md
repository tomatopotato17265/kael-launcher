...
All user-visible strings in Vue SFCs must use the localization system from `@kael/ui`. No hard-coded English strings should appear in templates or script — everything comes from `formatMessage` or `<IntlFormatted>`.
...
Messages are defined with `defineMessage` or `defineMessages` from `@kael/ui` in `<script setup>`. Each message has a unique `id` and a `defaultMessage` containing the English string:
...
Use `useVIntl()` from `@kael/ui` for simple string formatting:
...
For complex child handling, use `normalizeChildren` from `@kael/ui`:
...
All i18n utilities come from `@kael/ui`:
