import type { LoadingStateProvider } from '@kael/ui'
import { createLoadingStateCore, provideLoadingState } from '@kael/ui'

/**
 * Source of truth for the desktop app's loading state.
 *
 * Owns the token-based ref-counter directly (no Pinia store). Consumers
 * obtain the same reactive state via `injectLoadingState()` from `@kael/ui`.
 *
