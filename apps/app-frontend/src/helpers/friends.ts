import type { User } from '@kael/utils'
...
export async function transformFriends(friends: UserFriend[]): Promise<FriendWithUserData[]> {
	if (friends.length === 0) {
		return []
	}

	const friendStatuses = await friend_statuses()
	const users = await get_user_many(friends.map((x) => x.friend_id ?? x.id))
