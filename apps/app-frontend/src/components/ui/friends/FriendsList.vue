...
import { MailIcon, SendIcon, UserIcon, UserPlusIcon, XIcon } from '@kael/assets'
import {
	Avatar,
	ButtonStyled,
	defineMessages,
...
async function addFriend(friend: FriendWithUserData) {
	const id = friend.friend_id ?? friend.id
	if (id) {
...
async function loadFriends(timeout = false) {
	...
	try {
		...
		userFriends.value = await transformFriends(friendsList)
		loading.value = false
	} catch (e) {
...
<template>
	...
	<div v-if="!loading" class="flex gap-1 items-center mb-3 -ml-1">
