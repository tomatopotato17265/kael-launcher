...
```bash
pnpm add @kael/api-client
```

Tauri apps also need the optional peer dependency:

```bash
pnpm add @kael/api-client @tauri-apps/plugin-http
```
...
```ts
import { AuthFeature, GenericModrinthClient, type Labrinth } from '@kael/api-client'
...
```ts
import { AuthFeature, CircuitBreakerFeature, NuxtCircuitBreakerStorage, NuxtModrinthClient } from '@kael/api-client'
...
import { AuthFeature, TauriModrinthClient } from '@kael/api-client'
...
```ts
import { AuthFeature, CircuitBreakerFeature, RetryFeature } from '@kael/api-client'
...
```bash
pnpm --filter @kael/api-client build
pnpm --filter @kael/api-client lint
